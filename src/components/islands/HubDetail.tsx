  ...
  return (
    <div class="hub-detail">
      ...
      <style>{`
        ...
        .hub-detail-loading { display: flex; flex-direction: column; align-items: center; padding: 4rem 1rem; color: var(--text-muted, #6e7681); }
        .loading-pulse { width: 40px; height: 40px; border-radius: 50%; background: var(--accent-bright, #7cb2d4); animation: pulse 1.5s ease-in-out infinite; margin-bottom: 1rem; }
        @keyframes pulse { 0%,100% { opacity: 0.3; transform: scale(0.95); } 50% { opacity: 0.7; transform: scale(1.05); } }
        .hub-detail-empty { text-align: center; padding: 4rem 1rem; color: var(--text-secondary, #c9d1d9); }
        .hub-detail-empty a { color: var(--accent-bright, #7cb2d4); }

        .hd-header { margin-bottom: 2rem; }
        .hd-type-badge { display: inline-block; font-size: 0.75rem; font-family: var(--font-mono, monospace); color: var(--accent-bright, #7cb2d4); text-transform: uppercase; letter-spacing: 0.08em; margin-bottom: 0.75rem; }
        .hd-title { font-size: clamp(1.8rem, 5vw, 2.4rem); font-weight: 700; text-transform: capitalize; margin-bottom: 1rem; color: var(--text-primary, #e4eaf4); }
        .hd-breadcrumb { color: var(--text-secondary, #c9d1d9); font-size: 0.95rem; line-height: 1.6; margin-bottom: 1rem; }
        .hd-meta-row { display: flex; gap: 1rem; flex-wrap: wrap; }
        .hd-meta { font-family: var(--font-mono, monospace); font-size: 0.75rem; color: var(--text-muted, #6e7681); }
        ...
        .tier-core { background: rgba(124, 178, 212, 0.2); color: #7cb2d4; }
        ...
        .hd-tag { font-family: var(--font-mono, monospace); font-size: 0.7rem; padding: 2px 0.5rem; border-radius: 4px; background: rgba(124, 178, 212, 0.1); color: #7cb2d4; }

        .hd-stats { display: flex; flex-wrap: wrap; gap: 0.5rem; padding: 1rem 1.5rem; border-radius: 12px; border: 1px solid rgba(228,234,244,0.08); background: rgba(22,27,34,0.6); margin-bottom: 1.5rem; }
        .hd-stat { display: flex; flex-direction: column; gap: 0.2rem; padding-right: 1.5rem; border-right: 1px solid rgba(228,234,244,0.08); }
        .hd-stat:last-child { border-right: none; padding-right: 0; }
        .hd-stat-label { font-family: var(--font-mono, monospace); font-size: 0.65rem; color: var(--text-muted, #6e7681); text-transform: uppercase; letter-spacing: 0.06em; }
        .hd-stat-value { font-size: 0.9rem; font-weight: 600; color: var(--text-primary, #e4eaf4); }

        .hd-install { padding: 1.5rem; border-radius: 12px; border: 1px solid rgba(228,234,244,0.08); background: rgba(22,27,34,0.6); margin-bottom: 1.5rem; }
        .hd-install-cmd { display: block; font-family: var(--font-mono, monospace); font-size: 0.9rem; padding: 1rem; border-radius: 8px; background: #0d1117; color: #7cb2d4; margin-bottom: 0.5rem; }
        .hd-install-hint { font-size: 0.8rem; color: var(--text-muted, #6e7681); }

        .hd-includes { margin-bottom: 2rem; }
        .hd-includes h2 { font-size: 1.25rem; font-weight: 600; margin-bottom: 1rem; color: var(--text-primary, #e4eaf4); }
        .hd-card-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(220px, 1fr)); gap: 1rem; margin-bottom: 1rem; }
        .hd-inc-card { display: flex; flex-direction: column; padding: 1rem 1.25rem; border-radius: 12px; border: 1px solid rgba(228,234,244,0.08); border-left: 3px solid var(--card-accent, rgba(228,234,244,0.08)); background: rgba(22,27,34,0.6); text-decoration: none; color: var(--text-primary, #e4eaf4); transition: all 0.2s ease; }
        .hd-inc-card:hover { border-color: rgba(124, 178, 212, 0.3); transform: translateY(-2px); box-shadow: 0 8px 24px rgba(124, 178, 212, 0.08); }
        .hd-inc-top { display: flex; justify-content: space-between; align-items: baseline; margin-bottom: 0.5rem; }
        .hd-inc-type { font-family: var(--font-mono, monospace); font-size: 0.65rem; color: var(--accent-bright, #7cb2d4); text-transform: uppercase; letter-spacing: 0.06em; }
        .hd-inc-version { font-family: var(--font-mono, monospace); font-size: 0.65rem; color: var(--text-muted, #6e7681); }
        .hd-inc-name { font-size: 0.95rem; font-weight: 600; text-transform: capitalize; margin-bottom: 0.5rem; }
        .hd-inc-desc { font-size: 0.8rem; color: var(--text-secondary, #c9d1d9); line-height: 1.5; flex: 1; display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; overflow: hidden; }
        ...
        .hd-body { padding: 2rem; border-radius: 12px; border: 1px solid rgba(228,234,244,0.08); background: rgba(22,27,34,0.6); margin-bottom: 1.5rem; }
        .hd-body h2 { font-size: 1.25rem; font-weight: 600; margin: 1.5rem 0 1rem; padding-top: 1rem; border-top: 1px solid rgba(228,234,244,0.08); color: var(--text-primary, #e4eaf4); }
        ...
        .hd-body p { color: var(--text-secondary, #c9d1d9); line-height: 1.6; margin-bottom: 1rem; font-size: 0.9rem; }
        .hd-body ul, .hd-body ol { color: var(--text-secondary, #c9d1d9); padding-left: 1.5rem; margin-bottom: 1rem; font-size: 0.9rem; }
        .hd-body li { margin-bottom: 0.3rem; line-height: 1.6; }
        .hd-body code { font-family: var(--font-mono, monospace); font-size: 0.8rem; padding: 1px 0.3rem; border-radius: 4px; background: rgba(124, 178, 212, 0.1); color: #7cb2d4; }
        .hd-body pre { background: #0d1117; padding: 1rem; border-radius: 8px; overflow-x: auto; margin-bottom: 1rem; font-size: 0.8rem; line-height: 1.5; }
        .hd-body pre code { background: none; padding: 0; color: var(--text-secondary, #c9d1d9); }
        .hd-body table { width: 100%; border-collapse: collapse; margin-bottom: 1rem; font-size: 0.8rem; }
        .hd-body td { padding: 0.5rem; border-bottom: 1px solid rgba(228,234,244,0.08); color: var(--text-secondary, #c9d1d9); }
        .hd-body strong { color: var(--text-primary, #e4eaf4); }
        .hd-body blockquote { border-left: 3px solid #7cb2d4; padding-left: 1rem; margin: 1rem 0; color: var(--text-secondary, #c9d1d9); font-style: italic; }

        .hd-related { margin-bottom: 2rem; }
        .hd-related h2 { font-size: 1.25rem; font-weight: 600; margin-bottom: 1rem; color: var(--text-primary, #e4eaf4); }
        .hd-related-list { display: flex; flex-direction: column; gap: 0.5rem; }
        .hd-related-item { display: flex; align-items: baseline; gap: 1rem; padding: 0.75rem 1rem; border-radius: 8px; border: 1px solid rgba(228,234,244,0.08); background: rgba(22,27,34,0.4); text-decoration: none; color: var(--text-primary, #e4eaf4); transition: all 0.15s; }
        .hd-related-item:hover { border-color: rgba(124, 178, 212, 0.3); transform: translateX(4px); }
        .hd-related-type { font-family: var(--font-mono, monospace); font-size: 0.65rem; color: #7cb2d4; text-transform: uppercase; min-width: 4.5rem; }
        .hd-related-name { font-weight: 600; font-size: 0.9rem; min-width: 8rem; }
        .hd-related-desc { font-size: 0.8rem; color: var(--text-muted, #6e7681); flex: 1; }

        .hd-source { text-align: center; margin-bottom: 3rem; }
        .hd-source-link { font-size: 0.9rem; color: #7cb2d4; text-decoration: none; }
