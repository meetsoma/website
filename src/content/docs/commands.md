...
| `/exhale ["note"]` | Save state to disk. Writes `preload-next-<date>-<id>.md` to `memory/preloads/`, saves heat state with decay for unused content. Signals shared preload lifecycle (state → `REQUESTED`) so auto-rotation safety net defers. Session ends. Preload is what `soma inhale` loads next time. **Optional note:** text after `/exhale` is injected as `### User's Note for Next Session` in the preload template — the agent writing the preload puts this direction at the top of Start Here, overriding inferred next steps. (v0.28.1) |
